...
De esta manera, se almacenan en memoria los valores únicos por cada columna para después ser asociados a su correspondiente registro. Esto elimina el almacenamiento de valores duplicados por columna, reduciendo, así, el uso de la memoria y el espacio en disco requeridos para almacenar los datos. \cite{compression,dictionary-encoding}
...
\begin{itemize}
  ...
  \item {\itshape Series:} El particionamiento por serie segmenta los datos en una secuencia de particiones alocadas de manera dinámica. \cite{tables}
\end{itemize}
...
Finalmente, la operación \verb|DELETE| permite eliminar registros de una tabla mediante el formato \verb|DELETE FROM...WHERE|. \cite{sql-support}
...
Dependiendo del tipo de {\itshape join} usado, la memoria se verá afectada positiva o negativamente. Kinetica busca constantemente la forma de crear el {\itshape join} más eficiente posible. \cite{sql-support}
...
Debido a la potencia que le brinda la compatibilidad con  GPU, Kinetica rara vez se usa para el manejo de transacciones. Como sistema OLAP, es mejor aprovechado para consultas de análisis de datos junto con otro DBMS de tipo OLTP. \cite{faq}
...
En la Fig. \ref{fig:Niveles de almacenamiento} se muestra un diagrama de los diferentes niveles de almacenamiento.

% Missing citation
\begin{figure}[ht]
  \centering
  \includegraphics[width=75mm, scale=0.5]{./images/storage-tiers}
  \caption{Niveles de almacenamiento}
  \Description{Niveles de almacenamiento}
  \label{fig:Niveles de almacenamiento}
\end{figure}

El rendimiento de cada {\itshape tier} es inversamente proporcional a su capacidad de almacenamiento.  

\subsection{Manejo de datos históricos}

Kinetica cuenta con un {\itshape streaming data warehouse}, el cual, a diferencia de los {\itshape data warehouses} tradicionales, puede analizar los datos almacenados en tiempo real. De este modo, se obtiene un anáisis que puede ser utilizado de manera innmediata. \cite{streaming-data-warehouse}
...
Para la realización de esta consulta se realizó el query \verb|SELECT * FROM ki.home.world_population|. Se obtuvo un tiempo de ejecución de {\itshape 0.159s} y un tiempo de transferencia de datos de {\itshape 0.267s}.

\begin{figure}[ht]
  \centering
  \includegraphics[width=95mm, scale=0.7]{./images/query-select.png}
  \caption{Consulta sobre tabla completa}
  \Description{Consulta sobre tabla completa}
  \label{fig:Consulta sobre tabla completa}
\end{figure}

\subsection{Consulta haciendo uso de funciones de agregación y agrupamiento}

Para la realización de esta consulta se hizo uso de las funciones {\itshape COUNT(), SUM() y GROUP  BY}. El query ejecutado fue el siguiente \verb|SELECT COUNT(id) AS 'Total_by_gender', SUM(id),| \verb|gender FROM ki_home.world_population GROUP BY gender|. Se obtuvo un tiempo de ejecución de {\itshape 1.968s} y un tiempo de transferencia de datos de {\itshape 0.02s}.

\begin{figure}[ht]
  \centering
  \includegraphics[width=95mm, scale=0.7]{./images/query-aggregation.png}
  \caption{Consulta con funciones de agregación y agrupamiento}
  \Description{Consulta con funciones de agregación y agrupamiento}
  \label{fig:Consulta con funciones de agregacion y agrupamiento}
\end{figure}

\subsection{Consulta con condición de filtrado}

Por último, para la realización de la consulta con condición de filtrado se ejecutó el siguiente query \verb|SELECT * FROM ki_home.world_population WHERE name = 'Daniel'|. En esta consulta se obtuvo un tiempo de ejecución de {\itshape 14.218s} y un tiempo de transferencia de datos de {\itshape 0.454s}, siendo estos los tiempos más elevados de las tres consultas.

\begin{figure}[ht]
  \centering
  \includegraphics[width=95mm, scale=0.7]{./images/query-where.png}
  \caption{Consulta con condición de filtrado}
  \Description{Consulta con condición de filtrado}
  \label{fig:Consulta con condicion de filtrado}
\end{figure}
...
\begin{thebibliography}{9}
  \bibitem{faq} 
  «{\itshape Frequently Asked Questions}». https://www.kinetica.com/product/faq/
  \bibitem{overview} 
  «{\itshape Overview}».
  https://www.kinetica.com/docs/overview/arch.html
  \bibitem{compression}
  «{\itshape Compression}». https://www.kinetica.com/docs/concepts/compression.html
  \bibitem{dictionary-encoding}
  «{\itshape Dictionary Encoding}». https://www.kinetica.com/docs/concepts/compression.html
  \bibitem{tables}
  «{\itshape Tables}». https://www.kinetica.com/docs/concepts/tables.html
  \bibitem{sql-support}
  «{\itshape SQL Support}». https://www.kinetica.com/docs/concepts/sql.html
  \bibitem{streaming-data-warehouse}
  «{\itshape What Is a Streaming Data Warehouse?}». https://www.kinetica.com/products/why-streaming-data-warehouse/
\end{thebibliography}
