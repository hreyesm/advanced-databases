...
def main():
    ...
    #Inserción de 1ra parte de relaciones
    ...
    #Inserción de 2da parte de relaciones
    _ = db.run(':auto USING PERIODIC COMMIT\nLOAD CSV WITH HEADERS FROM "file:///soc-pokec-relationships2.csv" AS row\nMATCH (start:Profile {userID: row.start_id})\nMATCH (end:Profile {userID: row.end_id})\nMERGE (start)-[:FRIENDS_WITH]->(end);')

    #Inserción de 3ra parte de relaciones
    _ = db.run(':auto USING PERIODIC COMMIT\nLOAD CSV WITH HEADERS FROM "file:///soc-pokec-relationships3.csv" AS row\nMATCH (start:Profile {userID: row.start_id})\nMATCH (end:Profile {userID: row.end_id})\nMERGE (start)-[:FRIENDS_WITH]->(end);')
