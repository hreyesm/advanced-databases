...
// Importar las relaciones
    // Primer archivo
    :auto USING PERIODIC COMMIT
    LOAD CSV WITH HEADERS FROM "file:///pokec/soc-pokec-relationships-1.csv" AS row
    MATCH (start:Profile {userID: row.start_id})\nMATCH (end:Profile {userID: row.end_id})\nMERGE (start)-[:FRIENDS_WITH]->(end);

    // Segundo archivo
    :auto USING PERIODIC COMMIT
    LOAD CSV WITH HEADERS FROM "file:///pokec/soc-pokec-relationships-2.csv" AS row
    MATCH (start:Profile {userID: row.start_id})\nMATCH (end:Profile {userID: row.end_id})\nMERGE (start)-[:FRIENDS_WITH]->(end);

    // Tercer archivo
    :auto USING PERIODIC COMMIT
    LOAD CSV WITH HEADERS FROM "file:///pokec/soc-pokec-relationships-3.csv" AS row
    MATCH (start:Profile {userID: row.start_id})\nMATCH (end:Profile {userID: row.end_id})\nMERGE (start)-[:FRIENDS_WITH]->(end);